    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 sm:gap-6">
        ...
        <motion.div
          ...
          className="md:col-span-7 rounded-2xl sm:rounded-3xl border border-white/10 bg-neutral-950/70 p-5 sm:p-8 backdrop-blur-xl relative overflow-hidden flex flex-col justify-between"
        >
          <div className="relative z-10">
            <div className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-cyan-400 mb-5 sm:mb-6 text-xl">
              <FiCpu />
            </div>
        ...
        <motion.div
          ...
          className="md:col-span-12 rounded-2xl sm:rounded-3xl border border-white/10 bg-neutral-950/70 p-5 sm:p-8 backdrop-blur-xl relative"
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {techStack.map((tech, idx) => (
              <div
                key={idx}
                className={`p-3.5 sm:p-4 rounded-xl sm:rounded-2xl border border-white/10 bg-white/[0.02] flex flex-col items-center text-center group cursor-pointer transition-all duration-300 ${tech.glow}`}
              >
                <div className={`text-3xl sm:text-4xl ${tech.color} mb-2 sm:mb-3 group-hover:scale-110 transition-transform duration-300`}>
                  {tech.icon}
                </div>
                <h4 className="font-semibold text-xs sm:text-sm text-white">{tech.name}</h4>
                <p className="text-[10px] sm:text-xs text-neutral-500 mt-0.5 sm:mt-1">{tech.desc}</p>
              </div>
            ))}
          </div>
        </motion.div>
